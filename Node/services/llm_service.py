...
def get_sleep_analysis(data_payload):
    ...
    4. 반드시 "어제보다 [차이값]만큼 [늘었네요/줄었네요]"라는 형식을 지키고, 오늘 수치({today})와 차이값을 한 문장에 섞어서 "3.5시간에서 2.5시간 줄었네요" 같은 비논리적인 문장을 만들지 마세요.
    ...
    9. 문장 끝에 '기, 함, 음' 등 어떠한 조사나 어미도 붙이지 말고, 반드시 '명사' 또는 '명사구'로만 끝내세요. >  [나쁜 예]: 물 마시기(X), 스마트폰 안 함(X), 명상을 합니다(X), [좋은 예]: 따뜻한 허브차 한 잔, 스마트폰 전원 차단, 10분간 정석 명상, 암막 커튼 활용
    10. 어제 데이터가 없다면 오늘 데이터를 기준으로 다정한 첫 가이드를 제공하세요
